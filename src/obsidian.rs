...
/// Create an Obsidian note from a processed video
pub fn create_note(vault_path: &PathBuf, processed: &ProcessedVideo) -> Result<PathBuf> {
    let date = chrono::Local::now().format("%Y-%m-%d").to_string();
    let slug = slugify(&processed.video.title);
    let filename = format!("{date} - {slug}.md");
    let dir = vault_path.join("Resources").join("YouTube");
...
fn slugify(title: &str) -> String {
    title
        ...
        .filter(|&c| c != '-' || true) // keep all dashes for now
        .collect::<String>()
        .trim_matches('-')
        .to_string()
}
